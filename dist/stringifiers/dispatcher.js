  ...
  var _ref8 = _asyncToGenerator(function* (parent, node, prefix, context, selections, tables, wheres, orders, batchScope, dialect) {
    ...
    if (node.sqlJoin) {
      ...
      if (parent) {
        ...
        const joinCondition = yield node.junction.sqlBatch.sqlJoin(`${q(node.junction.as)}`, q(node.as), node.args || {}, context);
        if (node.paginate) {
      ...
      const joinCondition1 = yield node.junction.sqlJoins[0](`${q(parent.as)}`, q(node.junction.as), node.args || {}, context);
      const joinCondition2 = yield node.junction.sqlJoins[1](`${q(node.junction.as)}`, q(node.as), node.args || {}, context);
...
exports.default = (() => {
  var _ref6 = _asyncToGenerator(function* (topNode, context, options) {
    ...
    let hasCount = (0, _lodash.some)(selections, function (str) {
      return (0, _lodash.includes)(str, '$total');
    });
